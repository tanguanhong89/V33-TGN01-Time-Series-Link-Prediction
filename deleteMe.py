...
from code.ml_helper.ml_helper import torch_helpers as mlt
import os
import copy
import numpy as np
...
class v33tgn01():
    ...
    def create_model(self):
        ...
        self.e_lstm_out_to_emb, e2 = mlt.create_linear_layers(layer_sizes=[embedding_dim, 300, 200, 200, embedding_dim],
                                                              device=device)
        self.e_lstm_out_to_time, e3 = mlt.create_linear_layers(layer_sizes=[embedding_dim, 300, 200, 200, 1],
                                                               device=device)
        ...
        learning_weights = [self.w_node_emb] + e1 + e2 + e3 + e4 + e5 + list(self.lstm_model.parameters())
    ...
    class _batch():
        ...
        def _batch_loss(self, batch_output, batch, neg_batch, diagnostics_mode=False):
            ...
            def _base_comparator(new, baseline, fn='sub'):
                denom = baseline.pow(2).mean()
                if fn == 'sub':
                    num = new.sub(baseline).pow(2).mean()
                else:
                    num = new.add(baseline).pow(2).mean()
                loss = num.div(denom)
                return loss

            def _pos_neg_base_comparator(new, pos_base, neg_base):
                pos = _base_comparator(new, pos_base, fn='sub')
                neg = _base_comparator(new, neg_base, fn='add')
                return pos.add(neg)
            ...
            for i in range(len(batch)):
                ...
                for fc in range(frame_cnt):
                    time_loss = _base_comparator(p_time_list[fc], y_time_list[fc])
                ...
                p_future_time = p_time_list[-1]
                y_future_time = batch[i]['y']['time'][-1]
                future_time_loss = _base_comparator(p_future_time, y_future_time)

                future_y, future_neg = batch[i]['y']['class'][-1], neg_batch[i]['y']['class'][-1]
                future_py_emb = p_emb_list[-1].reshape(peek, -1)
                future_y_emb = t.index_select(self.w_node_emb, dim=0, index=future_y)

                future_emb_loss = _base_comparator(future_py_emb, future_y_emb)
    ...
    def _fprop(self, d):
        ...
        for i in range(frame_count):
            ...
            out_time = self._use_est(self.e_lstm_out_to_time, dles, t.tanh)
            out_emb = self._use_est(self.e_lstm_out_to_emb, dles, t.tanh)
            ...
            if i == frame_count - 1:
                ...
                future_time = self._use_est(self.e_lh_out_to_ftime, hidden_cat, t.tanh)
                future_emb = self._use_est(self.e_lh_out_to_femb, hidden_cat, t.tanh)
